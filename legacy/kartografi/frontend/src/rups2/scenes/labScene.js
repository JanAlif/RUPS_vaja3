...
export default class LabScene extends Phaser.Scene {
  ...
  preload() {
    // ✅ Vite-friendly relativne poti
    for (let i = 1; i <= 11; i++) {
      const url = new URL(`../avatars/avatar${i}.png`, import.meta.url).href;
      this.load.image(`avatar${i}`, url);
    }

    const defaultUrl = new URL("../avatars/avatar1.png", import.meta.url).href;
    this.load.image("defaultPfp", defaultUrl);
  }
  ...
  create() {
    ...
    this.wall = this.add
      .rectangle(0, 0, width, Math.max(0, height - 150 * ui), 0xe8e8e8)
      .setOrigin(0);
    this.floor = this.add
      .rectangle(0, height - 150 * ui, width, 150 * ui, 0xd4c4a8)
      .setOrigin(0);
    ...
    this.tableSurface = this.add
      .rectangle(tableX, tableY + 15, tableWidth - 30, tableHeight - 30, 0xa0826d)
      .setOrigin(0.5, 0);
    ...
    this.interactiveZone = this.add
      .zone(tableX, tableY + tableHeight / 2, tableWidth, tableHeight)
    ...
    // ✅ isti user kot React AuthContext
    const rawUser = localStorage.getItem("kartografi:user");
    const user = rawUser ? JSON.parse(rawUser) : null;

    const username = user?.username || "Uporabnik";

    // ✅ avatarPath (npr. "avatar7") -> "avatar7"
    const avatarPath = typeof user?.avatarPath === "string" ? user.avatarPath : "";
    const m = avatarPath.match(/avatar(\d+)/i);
    const avatarIndex = m?.[1] ? Number(m[1]) : 1;
    const safeIndex = Number.isInteger(avatarIndex) && avatarIndex >= 1 && avatarIndex <= 11 ? avatarIndex : 1;

    const pfpKey = `avatar${safeIndex}`;

    this.avatarRadius = 20 * ui;
    this.avatarBorderThickness = 4 * ui;

    this.avatarBorder = this.add
      .circle(200 * ui, 55 * ui, this.avatarRadius + this.avatarBorderThickness, 0xcccccc)
      .setOrigin(0.5);

    this.avatarInner = this.add
      .circle(200 * ui, 55 * ui, this.avatarRadius, 0xffffff)
      .setOrigin(0.5);

    this.avatarImage = this.add
      .image(200 * ui, 55 * ui, pfpKey)
      .setDisplaySize(this.avatarRadius * 2, this.avatarRadius * 2)
      .setOrigin(0.5);

    this.avatarImage.setMask(this.avatarInner.createGeometryMask());

    this.greetingText = this.add
      .text(this.avatarBorder.x + 50 * ui, this.avatarBorder.y + 10 * ui, `Dobrodošel ${username}!`, {
    ...
    const makeButton = (label, color, hover, onClick) => {
      ...
      const txt = this.add
        .text(this.buttonWidth / 2, this.buttonHeight / 2, label, {
    ...
    this.scoreBtn = makeButton("Lestvica", 0x3399ff, 0x0f5cad, () =>
    ...
    this.menuButtons.add([this.examplesBtn, this.scoreBtn]);

    // Hamburger menu (BREZ profila)
    this.hamburger = this.add
      .text(width - 50, 40, "☰", {
        fontSize: `${34 * ui}px`,
        color: "#222",
        resolution: window.devicePixelRatio,
    ...
    const dropExamples = this.add
      .text(0, 0, "Primeri", { fontSize: "22px", color: "#222" })
      .setInteractive({ useHandCursor: true })
      .on("pointerdown", () => this.scene.start("ExamplesScene"));

    const dropScore = this.add
      .text(0, 40, "Lestvica", { fontSize: "22px", color: "#222" })
      .setInteractive({ useHandCursor: true })
      .on("pointerdown", () => this.scene.start("ScoreboardScene"));

    this.hamburgerMenu.add([dropExamples, dropScore]);
    ...
    this.instruction = this.add
      .text(width / 2, height / 2 - 80 * ui, "Klikni na mizo in začni graditi svoj električni krog!", {
    ...
    const updateResponsiveLayout = () => {
      ...
      const tableX2 = w / 2;
      const tableY2 = h / 2 + 50 * uiNow;
      const tableWidth2 = Math.min(520 * uiNow, w - 100);
      const tableHeight2 = Math.min(260 * uiNow, h * 0.5);

      if (this.tableTop) this.tableTop.setPosition(tableX2, tableY2).setSize(tableWidth2, 30);
      if (this.tableSurface)
        this.tableSurface
          .setPosition(tableX2, tableY2 + 15)
          .setSize(Math.max(0, tableWidth2 - 30), Math.max(0, tableHeight2 - 30));
      if (this.tableLegLeft)
        this.tableLegLeft.setPosition(tableX2 - tableWidth2 / 2 + 40, tableY2 + tableHeight2 / 2 + 20);
      if (this.tableLegRight)
        this.tableLegRight.setPosition(tableX2 + tableWidth2 / 2 - 40, tableY2 + tableHeight2 / 2 + 20);

      if (this.interactiveZone) {
        this.interactiveZone.setPosition(tableX2, tableY2 + tableHeight2 / 2);
        this.interactiveZone.setSize(tableWidth2, tableHeight2);
      }
      ...
      if (this.gridGraphics) {
        ...
        const gridStartX = tableX2 - (tableWidth2 - 30) / 2;
        const gridStartY = tableY2 + 15;
        const gridEndX = tableX2 + (tableWidth2 - 30) / 2;
        const gridEndY = tableY2 + 15 + (tableHeight2 - 30);

        if (gridEndX > gridStartX && gridEndY > gridStartY && gridSize > 0) {
          for (let x = gridStartX; x <= gridEndX; x += gridSize) {
            this.gridGraphics.lineBetween(x, gridStartY, x, gridEndY);
          }
      ...
      if (isMobile) {
        ...
        const startX = w - this.buttonWidth * 2 - 40 * uiNow;
        this.menuButtons.setPosition(startX, 40 * uiNow);

        this.examplesBtn.setPosition(0, 0);
        this.scoreBtn.setPosition(this.buttonWidth + 20 * uiNow, 0);

        const newFontSize = Math.round(20 * uiNow);
        [this.examplesBtn, this.scoreBtn].forEach((btnContainer) => {
  ...
  shutdown() {
    if (this.avatarImage && this.avatarImage.clearMask) {
      try {
        this.avatarImage.clearMask(true);
      } catch (e) {}
    }
  }
}