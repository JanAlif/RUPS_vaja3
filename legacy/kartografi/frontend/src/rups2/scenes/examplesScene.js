...
export default class ExamplesScene extends Phaser.Scene {
  ...
  preload() {
    // ✅ Vite-friendly relativne poti + brez šumnikov v keyjih
    const img = (key, relPath) => {
      const url = new URL(relPath, import.meta.url).href;
      this.load.image(key, url);
    };

    img("baterija", "../components/battery.png");
    img("upor", "../components/resistor.png");
    img("svetilka", "../components/lamp.png");
    img("stikalo-on", "../components/switch-on.png");
    img("stikalo-off", "../components/switch-off.png");
    img("zica", "../components/wire.png"); // ✅ namesto "žica"
    img("ampermeter", "../components/ammeter.png");
    img("voltmeter", "../components/voltmeter.png");
  }
  ...
  create() {
    ...
    this.add
      .text(panelWidth / 2, 50 * ui, "Primeri krogov", {
    ...
    this.examples = [
      {
        ...
        components: [
          ...
          { type: "zica", ...pos(-0.5, -0.5), angle: 0 },
          { type: "stikalo-on", ...pos(0.5, -0.5), angle: 0 },

          { type: "zica", ...pos(1.0, 0.0), angle: 90 },

          { type: "zica", ...pos(0.5, 0.5), angle: 0 },
          { type: "svetilka", ...pos(-0.5, 0.5), angle: 0 },
          { type: "zica", ...pos(-1.5, 0.5), angle: 0 },

          { type: "zica", ...pos(-2.0, 0.0), angle: 90 },
        ],
      },
...

      {
        ...
        components: [
          ...
          { type: "zica", ...pos(2.0, 0.0), angle: 90 },
          { type: "zica", ...pos(2.0, 1.0), angle: 90 },

          { type: "zica", ...pos(1.5, 1.5), angle: 0 },
          { type: "svetilka", ...pos(0.5, 1.5), angle: 0 },
          { type: "zica", ...pos(-0.5, 1.5), angle: 0 },
          { type: "zica", ...pos(-1.5, 1.5), angle: 0 },

          { type: "zica", ...pos(-2.0, 0.0), angle: 90 },
          { type: "zica", ...pos(-2.0, 1.0), angle: 90 },

          { type: "zica", ...pos(0.5, 0.0), angle: 90 },
...

      {
        ...
        description:
          "Dve žarnici sta vezani vzporedno. Na vsaki žarnici je enaka napetost, tok se razdeli po vejah.",
        components: [
          { type: "baterija", ...pos(-2.0, -0.5), angle: 0 },
          { type: "zica", ...pos(-1.0, -0.5), angle: 0 },
          { type: "stikalo-on", ...pos(0.0, -0.5), angle: 0 },
          { type: "zica", ...pos(1.0, -0.5), angle: 0 },
          { type: "zica", ...pos(2.0, -0.5), angle: 0 },

          { type: "zica", ...pos(2.5, 0.0), angle: 90 },
    ...
    // ----------------- NASLOV + OPIS -----------------
    this.exampleTitle = this.add.text(surfaceMargin + 40, verticalMargin + 10, "", {
      fontSize: `${Math.round(24 * ui)}px`,
      fontStyle: "bold",
      color: "#0f172a",
    });

    this.exampleDescription = this.add.text(surfaceMargin + 40, verticalMargin + 55, "", {
    ...
    const placeComponent = ({ type, x, y, angle = 0 }) => {
      const size = type === "baterija" ? 130 * ui : 100 * ui;
      const img = this.add.image(x, y, type).setOrigin(0.5).setDisplaySize(size, size);
      img.setAngle(angle);
      return img;
    };
    ...
    this.examples.forEach((example, i) => {
      ...
      bgBtn.fillRoundedRect(bx - (panelWidth - 40) / 2, by - buttonHeight / 2, panelWidth - 40, buttonHeight, 10 * ui);

      this.add
        .text(bx, by, example.title, {
          color: "#ffffff",
