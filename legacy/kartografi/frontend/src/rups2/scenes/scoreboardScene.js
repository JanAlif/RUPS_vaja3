// src/rups2/scenes/scoreboardScene.js
...
export default class ScoreboardScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    this.previousScene = data?.previousScene || localStorage.getItem("lastScene") || "LabScene";
  }

  preload() {
    // ✅ Vite-friendly avatar preload (relativno glede na to datoteko)
    const asset = (rel) => new URL(rel, import.meta.url).href;

    // v labScene imaš 1..11, tukaj si imel 1..14; pusti 14, če slike obstajajo
    for (let i = 1; i <= 14; i++) {
      this.load.image(`avatar${i}`, asset(`../avatars/avatar${i}.png`));
    }
  }

  create() {
    const { width, height } = this.scale;
    const uiScale = Phaser.Math.Clamp(Math.min(width / 1280, height / 720), 0.65, 1.1);
    ...
    const tableHeight = Math.max(220 * uiScale, Math.min(320 * uiScale, height * 0.45));
    ...
    const panelHeight = Math.min(480 * uiScale, height - floorHeight - safeMargin * 1.5);
    const panelX = (width - panelWidth) / 2;
    const panelY = Math.max(safeMargin, (height - floorHeight - panelHeight) / 2 - 10 * uiScale);
    ...
    // naslov
    this.add
      .text(width / 2, panelY + 35 * uiScale, "LESTVICA", {
        fontFamily: "Arial",
    ...
    // ✅ trenutno prijavljen user (Kartografi auth)
    const rawUser = localStorage.getItem("kartografi:user");
    const loggedUser = rawUser ? JSON.parse(rawUser) : null;
    const loggedUsername = loggedUser?.username || null;

    // "Nalagam..." tekst
    const loadingText = this.add
      .text(panelX + panelWidth / 2, panelY + panelHeight / 2 - 60 * uiScale, "Nalagam lestvico...", {
        fontFamily: "Arial",
    ...
    // ESC tipka
    this.input.keyboard.on("keydown-ESC", () => this.navigateBack());

    // gumb nazaj
    const backButton = this.add
      .text(panelX + 20 * uiScale, panelY + 20 * uiScale, "↩ Nazaj", {
        fontFamily: "Arial",
      ...
      .on("pointerdown", () => this.navigateBack());
  ...
  navigateBack() {
    if (this.previousScene) {
      this.scene.start(this.previousScene, { cameFromMenu: this.previousScene === "LabScene" });
      return;
    }
  ...
  // avatarPath iz backenda pretvorimo v phaser key (avatar1..avatar14)
  resolveAvatarKey(avatarPath) {
    // če backend že pošlje "avatar7" ali "avatar7.png"
    if (typeof avatarPath === "string" && avatarPath.length) {
      const m = avatarPath.match(/avatar(\d+)/i);
      if (m && m[1]) return `avatar${m[1]}`;
    }
    return "avatar1";
  }

  loadLeaderboard(layout, loggedUsername, loadingText) {
    // ✅ tvoj backend endpoint (iz elektroUserController.js)
    fetch("/api/rups2/users/leaderboard")
      .then((res) => {
        if (!res.ok) throw new Error("Napaka pri fetchu leaderboarda");
        return res.json();
      })
      .then((users) => {
        loadingText.destroy();
        ...
        if (!Array.isArray(users) || users.length === 0) {
          this.add
            .text(layout.panelX + layout.panelWidth / 2, layout.rowStartY, "Ni še rezultatov.", {
              fontFamily: "Arial",
        ...
        users.forEach((user, index) => {
          ...
          const avatarKey = this.resolveAvatarKey(user.avatarPath);

          // avatar
          this.add
            .image(layout.avatarX, y + layout.avatarSize / 2, avatarKey)
          ...
          // ime (highlight)
          const style =
            loggedUsername && user.username === loggedUsername
              ? { fontSize: `${layout.fontSize}px`, color: "#0f5cad", fontStyle: "bold" }
              : { fontSize: `${layout.fontSize}px`, color: "#222" };

          this.add.text(layout.nameX, y + 3, user.username, style);

          // ✅ elektro high score
          const score = user.elektro_highScore ?? 0;

          this.add
            .text(layout.scoreX, y + 3, `${score}`, {
              fontSize: `${layout.fontSize}px`,
              color: "#0044cc",
            })
            .setOrigin(1, 0);
